...
RUN cd FFmpeg && \
    LDFLAGS="-Wl,-Bsymbolic" CFLAGS=-fPIC ./configure --enable-cuda --enable-nonfree --disable-shared --enable-static \
    --enable-gpl --enable-version3 --disable-programs --disable-doc --disable-avdevice --disable-postproc --disable-avfilter \
    --disable-encoders --disable-muxers --disable-debug --enable-pic --extra-ldflags="-Wl,-Bsymbolic" --extra-cflags="-march=native" && \
    make -j$(nproc) && make install -j$(nproc)
...
RUN git clone https://github.com/HomeOfAviSynthPlusEvolution/L-SMASH-Works && cd L-SMASH-Works && \
    cd VapourSynth/ && CFLAGS=-fPIC CXXFLAGS=-fPIC LDFLAGS="-Wl,-Bsymbolic" meson build && CFLAGS=-fPIC \
    CXXFLAGS=-fPIC LDFLAGS="-Wl,-Bsymbolic" ninja -C build && ninja -C build install 
...
RUN cd /usr/lib/x86_64-linux-gnu && ldconfig
...
# descale
RUN pip install meson ninja && git clone https://github.com/Irrational-Encoding-Wizardry/descale && \
    cd descale && meson build && ninja -C build && ninja -C build install #&& \
#########TODO??? cp ../descale.py /usr/local/lib/vapoursynth/
...
########################
# more vs plugins
# see https://github.com/abcnorio/docker-vapoursynth-encoder/tree/main
# fork and extended from https://git.gammaspectra.live/WeebDataHoarder/encoder
# numbers in brackets show the version originally used in the repo above

# OK - fieldhint v3
# /usr/local/lib/vapoursynth/fieldhint.so
RUN wget https://raw.githubusercontent.com/dubhater/vapoursynth-fieldhint/v3/src/fieldhint.c && \
    gcc -x c -I vapoursynth/include/ -shared -o /usr/local/lib/vapoursynth/fieldhint.so -O3 -fPIC fieldhint.c 

# ALREADY PRESENT - l-smash-works
#RUN git clone https://github.com/VFR-maniac/L-SMASH-Works.git && \
#    pushd L-SMASH-Works && \
#    cd VapourSynth && 
#    ./configure --vs-plugindir=/usr/local/lib/vapoursynth --prefix=/usr/local && \
#    make -j$(nproc) && make install

# OK - d2vsource ["r10.1"]
# /usr/local/lib/vapoursynth/libd2vsource.so
RUN git clone https://github.com/dwbuiten/d2vsource.git && cd d2vsource && \
    ./autogen.sh && ./configure --prefix=/usr/local && make -j$(nproc) && cp .libs/libd2vsource.so /usr/local/lib/vapoursynth/

# OK - VapourSynth-TDeintMod ["r10.1"]
# /usr/local/lib/vapoursynth/libtdeintmod.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-TDeintMod.git && cd VapourSynth-TDeintMod && \
    ./autogen.sh && ./configure && make -j$(nproc) && cp .libs/libtdeintmod.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - fmtconv ["r28"]
# /usr/local/lib/vapoursynth/libfmtconv.so
#RUN git clone https://github.com/EleonoreMizo/fmtconv.git && cd fmtconv && \
#    cd build/unix && ./autogen.sh && ./configure --prefix=/usr/local && make -j$(nproc) && \
#    cp .libs/libfmtconv.so /usr/local/lib/vapoursynth/

# OK - mvtools ["v23"]
# /usr/local/lib/vapoursynth/libmvtools.so
RUN git clone  https://github.com/dubhater/vapoursynth-mvtools.git && cd vapoursynth-mvtools && \
    ./autogen.sh && ./configure --prefix=/usr/local && make -j$(nproc) && cp .libs/libmvtools.so /usr/local/lib/vapoursynth/

# OK - FFT3DFilter ["64323f0fdee4dd4fe429ee6287906dbae8e7571c"]
# /usr/local/lib/vapoursynth/libfft3dfilter.so
RUN git clone https://github.com/myrsloik/VapourSynth-FFT3DFilter.git && cd VapourSynth-FFT3DFilter && \
    mkdir build && cd build && meson setup .. && ninja && cp libfft3dfilter.so /usr/local/lib/vapoursynth/

# OK - znedi3 ["r2.1"]
# /usr/local/lib/vapoursynth/vsznedi3.so
# /usr/local/lib/vapoursynth/nnedi3_weights.bin
RUN git clone --recursive https://github.com/sekrit-twc/znedi3 && cd znedi3 && \
    make X86=1 -j$(nproc) && cp nnedi3_weights.bin vsznedi3.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - descale ["r7"]
#RUN git clone https://github.com/Irrational-Encoding-Wizardry/descale.git && cd descale && \
#    mkdir build && cd build && meson setup .. && ninja && cp libdescale.so /usr/local/lib/vapoursynth/ && cp ../descale.py /usr/local/lib/vapoursynth/
#    /usr/local/lib/vapoursynth/libdescale.so
#    /usr/local/lib/vapoursynth/descale.py

# OK - flash3kyuu_deband ["master"]
# /usr/local/lib/vapoursynth/libf3kdb.so
# requires handling because of python >=3.11!!
# https://github.com/eddie3/gogrepo/issues/69
COPY build-vs-plugins/build_flash3kyuu_deband.sh /workplace/build_flash3kyuu_deband.sh
RUN /workplace/build_flash3kyuu_deband.sh
#
#!/bin/bash
...
# OK - BM3D ["r9"]
# /usr/local/lib/vapoursynth/libbm3d.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-BM3D.git && cd VapourSynth-BM3D && \
    mkdir build && cd build && LDFLAGS=-lfftw3f_threads meson setup .. && ninja && cp libbm3d.so /usr/local/lib/vapoursynth/

# OK - DFTTest ["r7"]
# /usr/local/lib/vapoursynth/libdfttest.so 
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-DFTTest.git && cd VapourSynth-DFTTest && \
    mkdir build && cd build && LDFLAGS=-lfftw3f_threads meson .. && ninja && cp libdfttest.so /usr/local/lib/vapoursynth/

# OK - Yadifmod ["r10.1"]
# /usr/local/lib/vapoursynth/libyadifmod.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-Yadifmod.git && cd VapourSynth-Yadifmod && \
    ./autogen.sh && ./configure --prefix=/usr/local && make -j$(nproc) && cp .libs/libyadifmod.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - FFMS2 ["2.40"]
# /usr/local/lib/vapoursynth/
#RUN git clone https://github.com/FFMS/ffms2.git && cd ffms2 && \
#    ./autogen.sh && ./configure --prefix=/usr/local && make -j$(nproc) && make install

# OK - AdddGrain ["master"]
# /usr/local/lib/vapoursynth/libaddgrain.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-AddGrain.git && cd VapourSynth-AddGrain && \
    meson build && ninja -C build && cp build/libaddgrain.so /usr/local/lib/vapoursynth/

# OK - Bwdif ["master"]
# /usr/local/lib/vapoursynth/libbwdif.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-Bwdif.git && cd VapourSynth-Bwdif && \
    meson setup build && ninja -C build && cp build/libbwdif.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - CAS ["master"]
# /usr/local/lib/vapoursynth/
#RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-CAS.git && cd VapourSynth-CAS && \
#    meson build &6 ninja -C build && cp build/libcas.so /usr/local/lib/vapoursynth/

# OK - CTMF ["master"]
# /usr/local/lib/vapoursynth/libctmf.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-CTMF.git && cd VapourSynth-CTMF && \
    meson setup build && ninja -C build && cp build/libctmf.so /usr/local/lib/vapoursynth/

# OK - DCTFilter ["master"]
# /usr/local/lib/vapoursynth/libdctfilter.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-DCTFilter.git && cd VapourSynth-DCTFilter && \
    meson setup build && ninja -C build && cp build/libdctfilter.so /usr/local/lib/vapoursynth/

# OK - Deblock ["master"]
# /usr/local/lib/vapoursynth/libdeblock.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-Deblock.git && cd VapourSynth-Deblock && \
    meson setup build && ninja -C build && cp build/libdeblock.so /usr/local/lib/vapoursynth/

# OK - EEDI2 ["master"]
# /usr/local/lib/vapoursynth/libeedi2.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-EEDI2.git && cd VapourSynth-EEDI2 && \
    meson setup build && ninja -C build && cp build/libeedi2.so /usr/local/lib/vapoursynth/

# OK - EEDI3 ["master"]
# /usr/local/lib/vapoursynth/libeedi3m.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-EEDI3.git && cd VapourSynth-EEDI3 && \
    meson setup build -Dopencl=false && ninja -C build && cp build/libeedi3m.so /usr/local/lib/vapoursynth/
# meson setup build -Dopencl=true && ninja -C build && cp build/libeedi3m.so /usr/local/lib/vapoursynth/
# requires opencl and boost, unclear which libboost-dev-?
# Debian bookworm: apt-get install cuda-opencl-12-1 cuda-opencl-dev-12-1 libboost-all-dev

# OK - fluxsmooth ["master"]
# /usr/local/lib/vapoursynth/libfluxsmooth.so
RUN git clone https://github.com/dubhater/vapoursynth-fluxsmooth.git && cd vapoursynth-fluxsmooth && \
    ./autogen.sh && ./configure && make && cp .libs/libfluxsmooth.so /usr/local/lib/vapoursynth/

# OK - hqdn3d ["master"]
# /usr/local/lib/vapoursynth/libhqdn3d.so
RUN git clone https://github.com/Hinterwaeldlers/vapoursynth-hqdn3d.git && cd vapoursynth-hqdn3d && \
    ./autogen.sh && ./configure && make && cp .libs/libhqdn3d.so /usr/local/lib/vapoursynth/

# OK - KNLMeansCL ["master"]
# /usr/local/lib/vapoursynth/libknlmeanscl.so
RUN apt-get update && apt-get install -y ocl-icd-opencl-dev libopencl-clang-dev libboost-all-dev && \
    git clone https://github.com/Khanattila/KNLMeansCL.git && cd KNLMeansCL && \
    meson setup build && ninja -C build && cp build/libknlmeanscl.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - miscfilters (obsolete) ["master"]
#RUN git clone https://github.com/vapoursynth/vs-miscfilters-obsolete.git && cd vs-miscfilters-obsolete && \
#    cd vs-miscfilters-obsolete && meson setup build && ninja -C build && cp build/libmiscfilters.so /usr/local/lib/vapoursynth/

# OK - NNEDI3CL ["master"]
# /usr/local/lib/vapoursynth/libnnedi3cl.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-NNEDI3CL.git && cd VapourSynth-NNEDI3CL && \
    meson setup build && ninja -C build && cp build/libnnedi3cl.so /usr/local/lib/vapoursynth/

# OK - removegrain ["master"]
# /usr/local/lib/vapoursynth/libremovegrain.so
RUN git clone https://github.com/vapoursynth/vs-removegrain.git && cd vs-removegrain && \
    meson setup build && ninja -C build && cp build/libremovegrain.so /usr/local/lib/vapoursynth/

# OK - sangnom ["master"]
# /usr/local/lib/vapoursynth/libsangnom.so
# fork, original not on the net anymore
RUN git clone https://github.com/dubhater/vapoursynth-sangnom.git && cd vapoursynth-sangnom && \
    meson setup build && ninja -C build && cp build/libsangnom.so /usr/local/lib/vapoursynth

# OK - sangnom modded ["master"]
# /usr/local/lib/vapoursynth/libsangnommod.so 
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-SangNomMod.git && cd VapourSynth-SangNomMod && \
    ./configure && make && cp ./libsangnommod.so /usr/local/lib/vapoursynth/

# OK - TTempSmooth ["master"]
# /usr/local/lib/vapoursynth/libttempsmooth.so
RUN git clone https://github.com/HomeOfVapourSynthEvolution/VapourSynth-TTempSmooth.git && cd VapourSynth-TTempSmooth && \
    meson setup build && ninja -C build && cp build/libttempsmooth.so /usr/local/lib/vapoursynth/

# ALREADY PRESENT - bestaudiosource ["master"]
#/usr/local/lib/vapoursynth/libbestaudiosource.so
#RUN git clone https://github.com/vapoursynth/bestaudiosource.git && cd bestaudiosource && \
#    curl https://raw.githubusercontent.com/vapoursynth/vapoursynth/master/include/VapourSynth4.h -O && \
#    meson setup build && ninja -C build && cp build/libbestaudiosource.so /usr/local/lib/vapoursynth/

# assrender ["master"]
# /usr/local/lib/x86_64-linux-gnu/libassrender.so
RUN apt-get update && apt-get install libass-dev -y && \
    git clone https://github.com/AmusementClub/assrender.git && cd assrender && \
    cmake -B build -S . && cmake --build build --clean-first && cp build/src/libassrender.so /usr/local/lib/x86_64-linux-gnu/
# note: not below /usr/local/lib/x86_64-linux-gnu/vapoursynth/libassrender.so

# OK
RUN wget https://raw.githubusercontent.com/HomeOfVapourSynthEvolution/havsfunc/3b6a80ce502e6775c70df6bb2d19124de149073f/havsfunc.py -O /usr/local/lib/vapoursynth/havsfunc.py
RUN wget https://raw.githubusercontent.com/HomeOfVapourSynthEvolution/mvsfunc/90e185220901402b974d7364345e0f25e71ab8fe/mvsfunc.py -O /usr/local/lib/vapoursynth/mvsfunc.py
RUN wget https://raw.githubusercontent.com/Irrational-Encoding-Wizardry/fvsfunc/29ced90a3f620844dea0efa8666542298f68f421/fvsfunc.py -O /usr/local/lib/vapoursynth/fvsfunc.py
RUN wget https://raw.githubusercontent.com/WolframRhodium/muvsfunc/6158bf2af1cb85fd9ba44024c52ef3666a2834c1/muvsfunc.py -O /usr/local/lib/vapoursynth/muvsfunc.py

# OK
RUN sed -i 's/core\.nnedi3/core\.znedi3/' /usr/local/lib/vapoursynth/fvsfunc.py
...
RUN apt-get update && apt-get install libwebp-dev -y && \
    ...
    ./configure --with-magick-plus-plus -with-webp=yes --disable-shared --enable-delegate-build --prefix=/usr/local && \
    make -j$(nproc) && make install

# OK - zenlib for mediainfo
RUN git clone https://github.com/MediaArea/ZenLib.git ZenLib && \
    cd ZenLib/Project/GNU/Library && \
    ./autogen.sh && ./configure --enable-static --prefix=/usr/local && \
    make -j$(nproc) && make install

# OK - mediainfolib
RUN git clone https://github.com/MediaArea/MediaInfoLib.git MediaInfoLib && \
    cd MediaInfoLib/Project/GNU/Library && \
    ./autogen.sh && ./configure --enable-staticlibs --enable-static --prefix=/usr/local && \
    make -j$(nproc) && make install

# OK - mediainfo
RUN apt-get install  && \
    git clone https://github.com/MediaArea/MediaInfo.git MediaInfo && \
    cd MediaInfo/Project/GNU/CLI && ./autogen.sh && ./configure --enable-staticlibs --prefix=/usr/local && \
    make -j$(nproc) && make install
...
RUN pip install tensorrt==9.3.0.post12.dev1 --pre tensorrt --extra-index-url https://pypi.nvidia.com/ && pip install polygraphy \
    --extra-index-url https://pypi.nvidia.com/ && \
    rm -rf /root/.cache/ /usr/local/lib/python3.11/site-packages/tensorrt_libs/libnvinfer.so.* \
    /usr/local/lib/python3.11/site-packages/tensorrt_libs/libnvinfer_builder_resource.so.* \
    /usr/local/lib/python3.11/site-packages/tensorrt_libs/libnvinfer_plugin.so.* \
    /usr/local/lib/python3.11/site-packages/tensorrt_libs/libnvonnxparser.so.*
...
# build of mkvtoolnix taken from WeebDataHoarder @ https://git.gammaspectra.live/WeebDataHoarder/encoder
# use that part of the Docker build because failed to build mkvtoolnix with qt6 statically (dynamically it went fine)
#/usr/local/bin/mkv*
FROM debian:bullseye AS build-mkvtoolnix
...
COPY build-weebdatahoarder/qt5/build-qt5.sh /build-qt5.sh
COPY build-weebdatahoarder/mkvtoolnix/build.sh /build.sh
RUN /build-qt5.sh
RUN /build.sh
...
COPY --from=base /workspace/Python-3.11.3/libpython3.11.so* \
    /workspace/Python-3.11.3/libpython3.so \
...
COPY --from=bestsource-lsmash-ffms2-vs /workspace/L-SMASH-Works/VapourSynth/build/libvslsmashsource.so \
    /workspace/bestsource/build/libbestsource.so /usr/local/lib/vapoursynth/
COPY --from=bestsource-lsmash-ffms2-vs /workspace/ffms2/src/core/.libs/libffms2.so* /usr/lib/x86_64-linux-gnu/

COPY --from=base /usr/local/lib/vapoursynth/libvmaf.so \
    /usr/local/lib/vapoursynth/libdescale.so \
    /usr/local/lib/vapoursynth/libakarin.so \
    /usr/local/lib/vapoursynth/libmiscfilters.so \
    /usr/local/lib/vapoursynth/libcas.so \
    /usr/local/lib/vapoursynth/
...
# /usr/local/lib/x86_64-linux-gnu/vapoursynth/libassrender.so

    /usr/local/lib/vapoursynth/havsfunc.py \
    /usr/local/lib/vapoursynth/mvsfunc.py \
    /usr/local/lib/vapoursynth/fvsfunc.py \
    /usr/local/lib/vapoursynth/muvsfunc.py \

    /usr/local/lib/vapoursynth/

############################ what is left...
# CHECK??? redundant copy, but not to forget anything...
COPY --from=base /usr/local/lib/x86_64-linux-gnu/* /usr/local/lib/x86_64-linux-gnu/
...
# imagemagick
COPY --from=base /usr/local/bin/magick /usr/local/bin/magick
COPY --from=base /usr/local/etc/ImageMagick-7 /usr/local/etc/ImageMagick-7
COPY --from=base /usr/local/bin/Magick* /usr/local/bin/
RUN ln -s /usr/local/bin/magick /usr/local/bin/animate && \
    ...
    ln -s /usr/local/bin/magick /usr/local/bin/ && \

# FROM SCRATCH
# ??? copy all to preserver symlinks, not just tools here...
# CHECK with ncdu!!!
...
# mediainfolib
# mediainfo
COPY --from=base /usr/local/bin/mediainfo /usr/local/bin/mediainfo

############################
# mkvtoolnix
COPY --from=build-mkvtoolnix /usr/local/bin/mkv* /usr/local/bin/
...
# trt
#COPY --from=TensorRT-ubuntu /usr/local/tensorrt/lib/libnvinfer_plugin.so* \
    /usr/local/tensorrt/lib/libnvinfer_vc_plugin.so* \
    /usr/local/tensorrt/lib/libnvonnxparser.so* \
    /usr/lib/x86_64-linux-gnu/
COPY --from=TensorRT-ubuntu /usr/local/tensorrt/targets/x86_64-linux-gnu/lib/libnvinfer_plugin.so* \
    /usr/local/tensorrt/targets/x86_64-linux-gnu/lib/libnvinfer_vc_plugin.so* \
    /usr/local/tensorrt/targets/x86_64-linux-gnu/lib/libnvonnxparser.so* \
    /usr/lib/x86_64-linux-gnu/

COPY --from=TensorRT-ubuntu /usr/local/cudnn/lib/libcudnn*.so* \
    /usr/local/tensorrt/lib/libnvinfer.so* \
    /usr/local/tensorrt/lib/libnvinfer_builder_resource.so* \
    /usr/local/tensorrt/lib/libnvonnxparser.so* \
    /usr/local/tensorrt/lib/libnvparsers.so.8* \
    /usr/local/tensorrt/lib/libnvinfer_plugin.so.8* \
    /usr/lib/x86_64-linux-gnu/

# ffmpeg (todo: try to make it fully static)
COPY --from=base /usr/lib/x86_64-linux-gnu/libxcb*.so* /usr/lib/x86_64-linux-gnu/libgomp*.so* \
...
COPY --from=base /usr/lib/x86_64-linux-gnu/libGL.so* \
...
# torch
COPY --from=torch-ubuntu /usr/lib/x86_64-linux-gnu/libopenblas.so* \
...
# FROM SCRATCH
#
# + investigate last image build:
# docker run --rm -it last-ID
#
# build with Dockerfile:
# docker build -t vsgantensorrt .
#
# run (+add whatever is required like shared folders):
# docker run --rm -it vsgantensorrt

############################
# TODO
#
# add more av tools and encoders
# add dgmpeg2dec + wine
# add vsmpeg
#
# last status quo - 2024-04-12
